...
function MaxOutUIPlugin:init()
    local ok, err = pcall(function()
        ...
        if p_root then
            ...
            if ok and type(meta) == "table" and (meta.name == "maxoutui" or (meta.name == "maxoutui" or meta.name == "simpleui")) then
                current_version = meta.version
            end
        end
        if not current_version then
            local meta_ok, meta = pcall(require, "_meta")
            if meta_ok and type(meta) == "table" and (meta.name == "maxoutui" or meta.name == "simpleui") then
                current_version = meta.version
            end
        end
            ...
            UIManager:scheduleIn(0, function()
                ...
                if ok_manga and Manga then
                    FM.instance:addFileDialogButtons("mui_pinned_manga", function(file, is_file, _book_props)
                        ...
                        return { Manga.genPinnedMangaButton(file, close_refresh) }
                    end)
                end
...
function MaxOutUIPlugin:onTeardown()
    ...
    local mod_manga = package.loaded["desktop_modules/module_manga"]
    if mod_manga and type(mod_manga.reset) == "function" then
        pcall(mod_manga.reset)
    end
