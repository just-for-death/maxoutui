...
function M.getCoverBB(filepath, w, h, align, stretch_limit)
    ...
    if cached then
        if cached == _NO_COVER then return nil end
        cached.t = os.time()
        return cached.bb
    end
    ...
    if manga_cover and _lfsMode(manga_cover) == "file" then
        ...
        if manga_cover:match("%.bb$") then
            ...
            local ok_bb, rbb = pcall(RenderImage.renderImageFile, RenderImage, manga_cover, false, nil, nil)
            if ok_bb then raw_bb = rbb end
        end

        if raw_bb then
            local final_bb = _scaleBBToSlot(raw_bb, w, h, align, stretch_limit)
            if _bim_cover_count >= BIM_MAX_COVERS then _evictOldestCover() end
            _bim_cover_cache[key] = { bb = final_bb, t = os.time() }
            _bim_cover_count = _bim_cover_count + 1
            return final_bb
        end
    end
