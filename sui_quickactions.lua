...
local function _getBookOrbitInstance()
    ...
    local fm = FM and FM.instance
    if fm then
        if fm.bookorbit then return fm.bookorbit end
        if fm._modules and fm._modules.bookorbit then return fm._modules.bookorbit end
    end
    local RUI = package.loaded["apps/reader/readerui"]
    local rui = RUI and RUI.instance
    if rui then
        if rui.bookorbit then return rui.bookorbit end
        if rui._modules and rui._modules.bookorbit then return rui._modules.bookorbit end
    end
    for key, m in pairs(package.loaded) do
        if type(key) == "string" and key:find("bookorbit", 1, true) and type(m) == "table" then
            local inst = m.instance or m
            if type(inst) == "table" and (inst.openCatalogBrowser or inst.browseCatalog) then
                return inst
            end
        end
    end
    ...
    local builtins = {
        ...
        },
        {
            ...
            execute = function(ctx)
                ...
                if sw_plugin and sw_plugin.showHistory then
        ...
        },
        {
            ...
            execute = function(ctx)
                local sw_plugin = _getSuwayomiInstance()
                if sw_plugin and sw_plugin.showUpdates then
                    sw_plugin:showUpdates()
                else
                    local su = ctx.show_unavailable or _unavailToast
                    su(_("Suwayomi plugin not available."))
                end
        ...
        },
        {
            ...
            execute = function(ctx)
                local sw_plugin = _getSuwayomiInstance()
                if sw_plugin and sw_plugin.showDownloads then
                    sw_plugin:showDownloads()
                else
                    local su = ctx.show_unavailable or _unavailToast
                    su(_("Suwayomi plugin not available."))
                end
        ...
        },
        {
            ...
            execute = function(ctx)
                ...
                local target_path, target_title

                if ctx.ui and ctx.ui.document and ctx.ui.document.file then
                    target_path = ctx.ui.document.file
                    local props = ctx.ui.doc_settings and ctx.ui.doc_settings:readSetting("doc_props")
                    target_title = (props and props.title and props.title ~= "") and props.title or target_path:gsub(".*/", "")
                end

                local cover_path
                if not target_path then
                    local sw_plugin = _getSuwayomiInstance()
                    if sw_plugin and sw_plugin.stream_session and sw_plugin.stream_session.manga then
                        local manga = sw_plugin.stream_session.manga
                        target_path = "suwayomi://manga/" .. tostring(manga.id)
                        target_title = manga.title or tostring(manga.id)
                        pcall(function()
                ...
                if not target_path then
                    local InfoMessage = require("ui/widget/infomessage")
                    UIManager:show(InfoMessage:new{ text = _("No active book or manga to pin."), timeout = 2 })
                    return
                end

                local InfoMessage = require("ui/widget/infomessage")
                if Manga.isPinnedManga(target_path) then
                    Manga.removePinnedManga(target_path)
                    UIManager:show(InfoMessage:new{ text = _("Unpinned from Pinned Manga"), timeout = 2 })
                else
                    Manga.addPinnedManga(target_path, target_title, cover_path)
                    UIManager:show(InfoMessage:new{ text = _("Pinned to Pinned Manga"), timeout = 2 })
                end
            end,
        },
        {
            id    = "bookorbit_sync",
            label = _("BookOrbit: Sync Now"),
            icon  = Config.ICON.sync or Config.ICON.refresh,
            is_in_place = true,
            execute = function(ctx)
                local Dispatcher = require("dispatcher")
                local Event = require("ui/event")
                Dispatcher:sendEvent(Event:new("BookOrbitSyncNow"))
                local InfoMessage = require("ui/widget/infomessage")
                UIManager:show(InfoMessage:new{ text = _("BookOrbit sync triggered"), timeout = 2 })
            end,
        },
        {
            id    = "bookorbit_catalog",
            label = _("BookOrbit Downloader & Library"),
            icon  = Config.ICON.collections or Config.ICON.file,
            is_in_place = true,
            execute = function(ctx)
                local bo = _getBookOrbitInstance()
                if bo then
                    if bo.openCatalogBrowser then
                        bo:openCatalogBrowser(true)
                    elseif bo.browseCatalog then
                        bo:browseCatalog(true)
                    end
                else
                    pcall(function()
                        local Dispatcher = require("dispatcher")
                        local Event = require("ui/event")
                        Dispatcher:sendEvent(Event:new("BookOrbitOpenDashboard"))
                    end)
                end
            end,
        },
